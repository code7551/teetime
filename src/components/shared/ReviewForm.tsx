...
export default function ReviewForm({ bookingId, backUrl }: ReviewFormProps) {
		...
		const fetchData = async () => {
			try {
				...
				if (bookingRes.ok) {
					...
					// Fetch student name
					try {
						const studentRes = await fetch(`/api/users/${data.studentId}`, {
							headers,
						});
						if (studentRes.ok) {
							const studentData = await studentRes.json();
							setStudentName(
								studentData.nickname || studentData.displayName || "นักเรียน",
							);
						}
					} catch {
	...
	const handleMediaFilesChange = (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		const totalImageCount =
			existingImageUrls.length + imageFiles.length + newImages.length;
		...
		<div className="space-y-6 max-w-2xl mx-auto">
			...
			<div className="flex items-center gap-3">
				<div>
					...
					<p className="text-gray-500 mt-1">บันทึกผลการเรียนและคำแนะนำให้นักเรียน</p>
				</div>
				{isEditing && (
			...
			{saved && (
				<Card className="bg-green-50 border border-green-200">
					<CardBody className="p-3 flex flex-row items-center gap-2">
						...
						<p className="text-green-700 text-sm font-medium">บันทึกรีวิวสำเร็จ!</p>
					</CardBody>
				</Card>
			)}
				...
				<CardBody className="p-6 space-y-5">
					...
					<div>
								...
								{existingVideoUrl && !videoFile && (
									<div className="flex items-center gap-3 p-3 bg-blue-50 rounded-lg border border-blue-200">
										...
										<div className="flex-1 min-w-0">
											<p className="text-sm font-medium text-blue-800">วิดีโอ</p>
						...
						>
							...
							<p className="text-sm text-gray-500">คลิกเพื่อเพิ่มรูปภาพหรือวิดีโอ</p>
